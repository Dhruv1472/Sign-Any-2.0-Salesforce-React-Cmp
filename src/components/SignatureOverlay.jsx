    ...
    return (
        <div className="signature-overlay">
            {pageSignatures.map((field, idx) => {
                ...
                const canDelete = !isSubmitted && (
                    isSignatureField ? sessionSignedKeys.has(field.index) : sessionFilledKeys?.has(field.index)
                );

                return (
                    <div
                        key={uniqueKey}
                        className={isSignatureField ? "signature-position" : "field-position"}
                        style={{
                            position: "absolute",
                            left: `${field.xPercent}%`,
                            top: `${field.yPercent}%`,
                        ...
                        {isSignatureField ? (
                            <SignatureButton 
                                signature={{ ...field, disabled: field.disabled }} 
                                onSign={onSign} 
                                onDelete={onDelete} 
                                canDelete={canDelete} 
                            />
